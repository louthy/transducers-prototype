...
public abstract record SumTransducerAsync<X, Y, A, B> : TransducerAsync<Sum<X, A>, Sum<Y, B>>
{
}
...
public record LiftSumTransducerAsync<X, Y, A, B>(TransducerAsync<X, Sum<Y, B>> Left, TransducerAsync<A, Sum<Y, B>> Right) 
...
/// creation of intermediate aggregates.
/// </summary>
record LiftSumTransducerAsync2<X, Y, A, B>(TransducerAsync<Sum<X, A>, Sum<Y, B>> Transducer) 
    : SumTransducerAsync<X, Y, A, B>
{
    /// <summary>
    /// Transform the transducer using the reducer provided 
