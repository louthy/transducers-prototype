...
public static partial class Transducer
{
    ...
    public static Transducer<Env, Transducer<Unit, Sum<X, A>>> FlattenT<Env, X, A>(
        this Transducer<Env, Transducer<Unit, Sum<X, Transducer<Env, Transducer<Unit, Sum<X, A>>>>>> m) =>
        new FlattenTTransducer1<Env, X, A>(m);

    public static Transducer<Env, Transducer<Env, Sum<X, A>>> FlattenT<Env, X, A>(
...
record FlattenTTransducer1<Env, X, A>(
    ...
    Transducer<Env, Transducer<Unit, Sum<X, A>>>
{
    public Reducer<S, Env> Transform<S>(Reducer<S, Transducer<Unit, Sum<X, A>>> reduce) =>
        new Reduce<S>(FF, reduce);

    record Reduce<S>(
        Transducer<Env, Transducer<Unit, Sum<X, Transducer<Env, Transducer<Unit, Sum<X, A>>>>>> FF,
        Reducer<S, Transducer<Unit, Sum<X, A>>> Reducer) : 
        Reducer<S, Env>
    {
    ...
    record Reduce1<S>(Env Env, Reducer<S, Transducer<Unit, Sum<X, A>>> Reducer) : 
    ...
    record Reduce2<S>(Env Env, Reducer<S, Transducer<Unit, Sum<X, A>>> Reducer) : 
        Reducer<S, Sum<X, Transducer<Env, Transducer<Unit, Sum<X, A>>>>>
    {
        public override TResult<S> Run(
            ...
            t switch
            {
                ...
                SumLeft<X, Transducer<Env, Transducer<Unit, Sum<X, A>>>> =>
                    TResult.Complete(s),
    ...
    record Reduce3<S>(Reducer<S, Transducer<Unit, Sum<X, A>>> Reducer) 
        : Reducer<S, Transducer<Unit, Sum<X, A>>>
    {
        public override TResult<S> Run(
            TState st,
            S s,
            Transducer<Unit, Sum<X, A>> t) =>
            Reducer.Run(st, s, t);
    }

    public Transducer<Env, Transducer<Unit, Sum<X, A>>> Morphism =>
        this;
}

record FlattenTTransducer2<Env, X, A>(
    Transducer<Env, Transducer<Env, Sum<X, Transducer<Env, Transducer<Env, Sum<X, A>>>>>> FF) : 
    Transducer<Env, Transducer<Env, Sum<X, A>>>
{
    public Reducer<S, Env> Transform<S>(Reducer<S, Transducer<Env, Sum<X, A>>> reduce) =>
        new Reduce<S>(FF, reduce);

    record Reduce<S>(
        Transducer<Env, Transducer<Env, Sum<X, Transducer<Env, Transducer<Env, Sum<X, A>>>>>> FF,
        Reducer<S, Transducer<Env, Sum<X, A>>> Reducer) : 
        Reducer<S, Env>
    {
        public override TResult<S> Run(TState st, S s, Env value) =>
            FF.Transform(new Reduce1<S>(value, Reducer)).Run(st, s, value);
    ...
    record Reduce1<S>(Env Env, Reducer<S, Transducer<Env, Sum<X, A>>> Reducer) : 
    ...
    record Reduce2<S>(Env Env, Reducer<S, Transducer<Env, Sum<X, A>>> Reducer) : 
        Reducer<S, Sum<X, Transducer<Env, Transducer<Env, Sum<X, A>>>>>
    {
        public override TResult<S> Run(
            ...
            t switch
            {
                ...
                SumLeft<X, Transducer<Env, Transducer<Env, Sum<X, A>>>> =>
                    TResult.Complete(s),
    ...
    record Reduce3<S>(Reducer<S, Transducer<Env, Sum<X, A>>> Reducer) 
    ...
    public Transducer<Env, Transducer<Env, Sum<X, A>>> Morphism =>
        this;
}
