    ...
    Transducer<A, C>
{
    ...
    public override TransducerAsync<A, C> ToAsync() =>
        new BindTransducerAsync3A<A, B, C>(M.ToAsync(), x => F(x).ToAsync());
    ...
    internal record Reduce<S>(Transducer<A, B> M, Func<B, Transducer<Unit, C>> F, Reducer<S, C> Reducer) : Reducer<S, A>
    {
        ...
        public override ReducerAsync<S, A> ToAsync() =>
            new BindTransducerAsync3A<A, B, C>.Reduce<S>(M.ToAsync(), x => F(x).ToAsync(), Reducer.ToAsync());
    }
    ...
    internal record Binder<S>(Func<B, Transducer<Unit, C>> F, Reducer<S, C> Reducer) : Reducer<S, B>
    {
        ...
        public override ReducerAsync<S, B> ToAsync() =>
            new BindTransducerAsync3A<A, B, C>.Binder<S>(x => F(x).ToAsync(), Reducer.ToAsync());
    }
}
    ...
    Transducer<A, C>
{
    ...
    public override TransducerAsync<A, C> ToAsync() =>
        new BindTransducerAsync3B<A, B, C>(M.ToAsync(), x => F(x).ToAsync());
    ...
    internal record Reduce<S>(Transducer<Unit, B> M, Func<B, Transducer<A, C>> F, Reducer<S, C> Reducer) : Reducer<S, A>
    {
        ...
        public override ReducerAsync<S, A> ToAsync() =>
            new BindTransducerAsync3B<A, B, C>.Reduce<S>(M.ToAsync(), x => F(x).ToAsync(), Reducer.ToAsync());
    }
    ...
    internal record Binder<S>(A Value, Func<B, Transducer<A, C>> F, Reducer<S, C> Reducer) : Reducer<S, B>
    {
        ...
        public override ReducerAsync<S, B> ToAsync() =>
            new BindTransducerAsync3B<A, B, C>.Binder<S>(Value, x => F(x).ToAsync(), Reducer.ToAsync());
    }
}
